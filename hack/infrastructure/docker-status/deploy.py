...
def init_docker_status():
    ...
    login_str = ( 'http://{0}/_ah/login?email={1}&admin=True&action=Login&'
        'continue=http://{0}/admin'.format(
        docker_status_server, admin_login) )
    retry(r.get, [login_str], sleep=1, hide_exc=True)
    r.get('http://{}/admin'.format(docker_status_server))
    r.post('http://{}/admin/setup'.format(docker_status_server))
...
with warn_only():
    sudo('stop {}'.format(app_id))
sudo('start {}'.format(app_id))
run('for id in `docker ps | grep mzdaniel/docker-status  | cut -d" " -f1`; do '
    'docker kill $id; done'.format(repository))
